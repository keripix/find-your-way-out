...
GamePlay.prototype.onTick = function() {
  ...
  this.listenForKeyEvent(this.shell, this.actor);
};

/**
 * Listening for keyboard event
 *
 * @param  {GameShell} shell GameShell instance
 * @param  {Actor} actor Actor instance
 */
GamePlay.prototype.listenForKeyEvent = function(shell, actor) {
  if(shell.wasDown("move-left") && actor.stopperLocation !== "-x") {
    actor.moveDirection = {direction: "x", value: -1};
    ...
    actor.moveDirection = {direction: "y", value: -1};
    actor.requestRendering = true;
 ...
 * TODO The implementations assumes too much about what to render based
 * on the state of the actor.
 *
 * What if the implementation is delegated to the actor itself?
 */
GamePlay.prototype.onRender = function() {
  if (this.actor.requestRenderingForce){
    ...
    var moveDirection = this.actor.moveDirection,
        speed = this.game.speed * moveDirection.value;

    this.ctx.clearRect(this.actor.x, this.actor.y, this.actor.width, this.actor.height);

    this.ctx.fillStyle = this.actor.color;
    // eg this.actor.moveY
    this.actor["move" + moveDirection.direction.toUpperCase()].call(this.actor, speed);
    this.ctx.fillRect(this.actor.x, this.actor.y, this.actor.width, this.actor.height);
  }
};